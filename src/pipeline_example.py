...
def sentiment_analysis_example():
    ...
    # Display results
    for sentence, result in zip(sentences, results, strict=False):
        label = result["label"]
        score = result["score"]
...
def text_generation_example():
    ...
    generator = pipeline(
        "text-generation", model="distilgpt2", device=get_device(), max_length=50
    )
...
def zero_shot_classification_example():
    ...
    classifier = pipeline("zero-shot-classification", device=get_device())
...
def question_answering_example():
    ...
    # Create QA pipeline
    qa_pipeline = pipeline("question-answering", device=get_device())

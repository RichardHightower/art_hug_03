...
import torch
from config import get_device, MAX_LENGTH

def run_advanced_tokenization_examples():
    """Run advanced tokenization examples including padding, truncation, and special tokens."""
    ...
    texts = [
        ...
        "This is a much longer sentence that will show how padding works with multiple sentences of different lengths in a batch."
    ]
    ...
    print("\n2. Truncation Examples:")
    long_text = " ".join(["This is a very long sentence."] * 50)  # Create a very long text
    ...
    tokens_with_trunc = tokenizer(long_text, truncation=True, max_length=20, return_tensors="pt")
    print(f"   With truncation (max_length=20): {tokens_with_trunc['input_ids'].shape[1]} tokens")
    print(f"   Truncated tokens: {tokenizer.convert_ids_to_tokens(tokens_with_trunc['input_ids'][0])}")
    ...
    print(f"   With special tokens: {tokenizer.convert_ids_to_tokens(tokens_with_special['input_ids'])}")
    print(f"   Without special tokens: {tokenizer.convert_ids_to_tokens(tokens_without_special['input_ids'])}")
    ...
    context = "Tokenization is the process of breaking down text into smaller units called tokens."
    
    # Encode question and context together
    qa_encoding = tokenizer(question, context, padding=True, truncation=True, return_tensors="pt")
    ...
    fixed_padding = tokenizer(batch_texts, padding='max_length', max_length=30, return_tensors="pt")
    ...
    for token, (start, end) in zip(tokenizer.convert_ids_to_tokens(encoding['input_ids']), 

...
def basic_translation_example():
    ...
    for original, result in zip(sentences, translations, strict=False):
...
def multilingual_translation_example():
    ...
    for src_lang, tgt_lang, model_name in language_pairs:
        ...
        translator = pipeline(
            f"translation_{src_lang}_to_{tgt_lang}", model=model_name, device=device
        )
...
def reverse_translation_example():
    ...
    fr_to_en = pipeline(
        "translation_fr_to_en", model="Helsinki-NLP/opus-mt-fr-en", device=device
    )

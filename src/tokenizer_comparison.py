...
def run_tokenizer_comparison_examples():
    ...
    for text_name, text in test_texts.items():
        ...
        results.append(
            [
                ...
                (
                    tiktoken_tokens[:5] + ["..."]
                    if len(tiktoken_tokens) > 5
                    else tiktoken_tokens
                ),
                (
                    str(tiktoken_ids[:5]) + "..."
                    if len(tiktoken_ids) > 5
                    else str(tiktoken_ids)
                ),
            ]
        )
    ...
    print(
        tabulate(vocab_sizes, headers=["Tokenizer", "Vocabulary Size"], tablefmt="grid")
    )
    ...
    print(
        tabulate(
            perf_results,
            headers=["Tokenizer", "Time (100 iterations)"],
            tablefmt="grid",
        )
    )

...
def list_text_classification_models():
    ...
    sorted_models = sorted(model_list, key=lambda x: x.downloads or 0, reverse=True)[:10]
    
    for i, model in enumerate(sorted_models, 1):
        downloads = model.downloads or "N/A"
        likes = model.likes or 0
        print(f"{i:2d}. {model.modelId[:50]:<50} | Downloads: {downloads:>10} | Likes: {likes:>5}")
    print()
...
def list_models_by_library():
    ...
    pytorch_translation = list_models(
        task="translation",
        library="pytorch",
        limit=5
    )
...
def check_auth_status():
    ...
    if token:
        try:
            ...
            print(f"   Organizations: {', '.join(org['name'] for org in user.get('orgs', []))}")
        except Exception as e:
